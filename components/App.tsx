    ...
    <div className="min-h-screen bg-[#F8FAFC] flex font-sans antialiased text-slate-800 overflow-hidden">
      ...
      <div className={`fixed inset-y-0 left-0 w-64 bg-white z-50 transform transition-transform duration-300 ease-in-out md:hidden shadow-xl ${isMobileMenuOpen ? 'translate-x-0' : '-translate-x-full'}`}>
        ...
        <nav className="p-3 space-y-1">
          ...
          {/* Mobile direct link for templates */}
          <button
            onClick={() => window.open(TEMPLATES_URL, '_blank')}
            className="w-full text-left px-3 py-2 rounded-lg text-slate-600 hover:bg-slate-50 flex items-center justify-between group"
          >
            <span className="text-xs">Laiškų šablonai</span>
            <ExternalLink size={12} className="text-slate-400" />
          </button>

          {/* Mobile direct link for registrai */}
          <button
            onClick={() => window.open(REGISTRAI_URL, '_blank')}
            className="w-full text-left px-3 py-2 rounded-lg text-slate-600 hover:bg-slate-50 flex items-center justify-between group"
          >
            <span className="text-xs">Registrai</span>
            <ExternalLink size={12} className="text-slate-400" />
          </button>
        </nav>
      </div>
